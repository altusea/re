...
public class ShoppingCartDecider {
    ...
    public static ShoppingCart.Event decide(Command command, ShoppingCart state) {
        return switch (on(state, command)) {
            case FunctionalTools.On(ShoppingCart.Initial _, Command.Open(var id, var clientId, var now)) ->
            ...
            case FunctionalTools.On(ShoppingCart.Pending _, Command.Confirm(var id, var now)) ->
                    new ShoppingCart.Event.Confirmed(id, now);

            case FunctionalTools.On(ShoppingCart.Pending _, Command.Cancel(var id, var now)) ->
