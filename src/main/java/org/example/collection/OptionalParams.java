...
public class OptionalParams {
    ...
    public static <T extends Comparable<? super T>> void sortTable(T[][] table, int column) {
        sortTable(table, column, false);
    }

    public static <T extends Comparable<? super T>> void sortTable(T[][] table, int column, boolean reverse) {
        sortTable(table, column, reverse, OptionalParams.naturalOrdering());
    }
    ...
    public static <T> void sortTable(T[][] table,
        ...
        Comparator<T[]> myCmp = (x, y) -> (reverse ? -1 : 1) * ordering.compare(x[column], y[column]);
        Arrays.sort(table, myCmp);
    }

    public static void main(String[] args) {
        String[][] data0 = {{"a", "b", "c"}, {"", "q", "z"}, {"zap", "zip", "Zot"}};
        ...
        data = data0.clone();
        sortTable(data, 0, false, (a, b) -> b.length() - a.length());
        System.out.println(Arrays.deepToString(data));
        // prints: [[zap, zip, Zot], [a, b, c], [, q, z]]
    }
}
