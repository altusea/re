...
import org.example.util.ComparableUtils;
 ...
 */
public class CachedSupplier<T> implements Supplier<T>, AutoCloseable {
    ...
    public T get() {
        if (cacheIsStale()) {
            log.debug("({}) Cached value is stale and will be refreshed.", cachedValueName);
            refreshCache();
        } else if (shouldInitiateCachePrefetch()) {
            log.debug("({}) Cached value has reached prefetch time and will be refreshed.", cachedValueName);
            prefetchCache();
        }
        ...
        try {
            ...
            try {
                ...
                if (cacheIsStale() || shouldInitiateCachePrefetch()) {
                    log.debug("({}) Refreshing cached value.", cachedValueName);
                    ...
                    try {
                        ...
                        log.debug("({}) Successfully refreshed cached value. Next Prefetch Time: {}. Next Stale Time: {}", cachedValueName, cachedValue.prefetchTime(), cachedValue.staleTime());
    ...
    private RefreshResult<T> handleFetchedSuccess(RefreshResult<T> fetch) {
        ...
        switch (staleValueBehavior) {
            case STRICT:
                ...
                log.warn("({}) Retrieved value expiration is in the past ({}). Using expiration of {}", cachedValueName, fetch.staleTime(), newStale);
            ...
            case ALLOW:
                ...
                log.warn("({}) Cached value expiration has been extended to {} because the downstream service returned a time in the past: {}", cachedValueName, newStaleTime, fetch.staleTime());
    ...
    private RefreshResult<T> handleFetchFailure(RuntimeException e) {
        log.debug("({}) Failed to refresh cached value.", cachedValueName, e);
        ...
        if (!now.isBefore(currentCachedValue.staleTime())) {
            ...
            switch (staleValueBehavior) {
                ...
                case ALLOW:
                    ...
                    log.warn("({}) Cached value expiration has been extended to {} because calling the downstream service failed (consecutive failures: {}).", cachedValueName, newStaleTime, numFailures, e);
